...
import {
  ...
  computeSha256,
} from "../../../src/infrastructure/provider/ingestion/index.js";
import { IngestionCache } from "../../../src/infrastructure/provider/ingestion/cache.js";
import type { ExportBundleManifest } from "@combat-designer/backend";

function loadFixtureBundle(): {
  manifest: ExportBundleManifest;
  files: Map<string, string>;
} {
  let fixtureDir = path.resolve(process.cwd(), "src/infrastructure/provider/ingestion/fixtures/unity-bundle/assets");
  if (!fs.existsSync(fixtureDir)) {
    fixtureDir = path.resolve(process.cwd(), "backend/src/infrastructure/provider/ingestion/fixtures/unity-bundle/assets");
  }
  if (!fs.existsSync(fixtureDir)) {
    const dir = typeof __dirname !== "undefined" ? __dirname : path.dirname(fileURLToPath(import.meta.url));
    fixtureDir = path.resolve(dir, "../../../src/infrastructure/provider/ingestion/fixtures/unity-bundle/assets");
  }
