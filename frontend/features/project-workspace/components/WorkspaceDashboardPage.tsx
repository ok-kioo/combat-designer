...
function WorkspaceCard({ workspace: ws, reload }: { workspace: Workspace; reload: () => void }) {
  ...
  return <article className="card"><span className="badge">{ws.engine || 'Unity'} · {ws.status === 'archived' ? 'Arquivado' : 'Ativo'}</span><h2>{ws.name}</h2><p>{ws.description || 'Seu próximo sistema de combate começa aqui.'}</p><p className="muted">Atualizado em {new Date(ws.updated_at).toLocaleDateString('pt-BR')}</p>{error && <p role="alert" className="error">{error}</p>}{editing && <form onSubmit={e => { e.preventDefault(); void mutate(); }}><label>Novo nome<input value={name} onChange={e => setName(e.target.value)} required maxLength={120} autoFocus /></label><button type="submit" disabled={busy}>Salvar nome</button><button type="button" onClick={() => setEditing(false)}>Cancelar</button></form>}<div className="actions">{ws.status === 'active' && <><Link className="button primary" to={'/workspaces/' + encodeURIComponent(ws.id)}>Abrir projeto →</Link><button type="button" className="quiet" onClick={() => setEditing(true)}>Renomear</button><button type="button" className="quiet" disabled={busy} onClick={() => { if (confirm('Arquivar este projeto? O histórico será preservado.')) void mutate(true); }}>Arquivar</button></>}</div></article>;
}
