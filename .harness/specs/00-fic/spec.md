...
Nenhuma feature pode mudar de `status` para `released` sem `approval.gate_run_id` preenchido e um `GateResult` `PASS` vinculado a `approval.model_revision` e `approval.gate_rule_set_version`. Se o `model_revision` do repositório avançar após o `gate_run_id` ter sido emitido, a aprovação é considerada `STALE` e o gate deve ser reexecutado antes do release.
